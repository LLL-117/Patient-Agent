...
可选：单独启动 `mcp_server.py`（`uvicorn backend.mcp_server:app`），提供 **Streamable HTTP**（**`POST /mcp`** JSON-RPC，**`mcp_streamable_http.py`**）、旧版 REST（tools/invoke、agent-call、agent-react）及 QwenRouter；与主应用共用同一套数据模型思路，进程独立。
...
可选：mcp_server 独立进程 → Streamable HTTP POST /mcp（JSON-RPC）+ 旧 REST（tools/invoke、agent-call、agent-react）
```

对应 API（主应用，根路径为服务地址，如 `http://127.0.0.1:8000`）：静态页 `GET /test/`（挂载 `backend/static`）；其余 REST 与 Agent 见第 15 节。对应 API（独立 MCP 进程，`uvicorn backend.mcp_server:app`）：`GET /health`；**`POST /mcp`**（Streamable HTTP）、**`GET /mcp`**→405、**`DELETE /mcp`**；以及 `GET /mcp/tools`、`POST /mcp/invoke`、`POST /mcp/agent-call`、`POST /mcp/agent-react`。
...
**旧版 REST（仍保留）**：`GET /mcp/tools`、`POST /mcp/invoke` 直接调工具；`POST /mcp/agent-call` 走 `QwenRouter.route()` 单次规划再 invoke；`POST /mcp/agent-react` 多轮 ReAct。
...
对应 API（MCP 独立服务）：`GET /health`；**`POST /mcp`**（Streamable HTTP）；`GET /mcp`→405；`DELETE /mcp`（可选注销会话，带 `MCP-Session-Id`）；`GET /mcp/tools`、`POST /mcp/invoke`、`POST /mcp/agent-call`、`POST /mcp/agent-react`。主应用侧多步规划见 `POST /api/agent/react-plan`（第 8 节）。

> 面试表达：标准对接可走 **Streamable HTTP `POST /mcp`**（JSON-RPC `tools/list` / `tools/call`）；旧版 REST 仍可用。MCP 仍是工具注册表 + invoke、可选 Qwen 路由；单步多采样投票与多步 ReAct 分属不同接口。主 Agent 用轻量规则路由，与 MCP 演示共用 Toolbox，但刻意解耦。
...
问：MCP 和主 Agent 什么关系？  
答：工具实现可复用 `MCPToolbox`；主 Agent 是轻量规则加数据库查询；MCP 独立进程提供 **Streamable HTTP `POST /mcp`**（JSON-RPC）及旧 REST，侧重 LLM 选工具与多步规划演示。
...
这是一个 FastAPI 医疗 Agent 后端：用 SQLite 统一管理患者、病例、就诊与分层记忆；长期记忆通过**手动抽取**或**用量/定时自动抽取**（与对话抽取同源）写入关键事件与画像，并用 FTS5 与 DashScope Embedding 做混合检索；对外提供多模态问答（VL+TTS）与可选的 ReAct Planner；可选独立 MCP 进程通过 **Streamable HTTP `POST /mcp`**（JSON-RPC，非 stdio/SSE 响应体）暴露工具；工程上强调模块化路由、环境变量换模型、可解释的检索与记忆拼接。
...
独立 MCP 进程（非主应用端口，需单独 `uvicorn backend.mcp_server:app`）：`GET /health`；**`POST /mcp`**（Streamable HTTP，JSON-RPC）、**`GET /mcp`**→405、**`DELETE /mcp`**（带 `MCP-Session-Id`）；`GET /mcp/tools`、`POST /mcp/invoke`、`POST /mcp/agent-call`、`POST /mcp/agent-react`。
...
`mcp_server.py`：MCP 工具箱、QwenRouter、独立 FastAPI；挂载 **`POST /mcp`** Streamable HTTP。  
